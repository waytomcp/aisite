...
export default function GenericComparison({ data }: { data: CatalogItemData['comparison'] }) {
  return (
    <section id="comparison" className="w-full py-12 lg:py-16 bg-white dark:bg-gray-900 flex justify-center">
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800">
          <Table>
            ...
            <TableBody className="text-slate-500 dark:text-gray-300">
              {data.data.map((item) => (
                <TableRow key={item.id} className="border-slate-200 dark:border-gray-700 hover:bg-slate-50 dark:hover:bg-gray-700/50">
                  {data.columns.map((col) => (
                    <TableCell key={`${item.id}-${col.key}`} className={col.key === 'product' ? 'font-medium text-slate-900 dark:text-white' : ''}>
                      {item[col.key]}
                    </TableCell>
                  ))}
                </TableRow>
